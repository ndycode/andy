...
const PREFERENCE_MEMORY_RE =
  /\b(?:i\s+(?:like|love|prefer|usually|(?:always|often)\s+(?:get|order|drink|eat|buy|use|pay\s+with))|i\s+(?:do\s+not\s+like|don't\s+like|hate\s+(?!this\b|that\b|it\b|you\b|them\b))|my\s+(?:favou?rite|usual|default))\b/;
